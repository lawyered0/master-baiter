...
def resolve_persona(name: str) -> str:
    ...
    Handles hyphens, underscores, and spaces interchangeably so callers
    can pass 'The Counter-Scammer', 'counter_scammer', or 'counter scammer'.
    """
    key = name.lower().strip().replace("-", " ").replace("_", " ")
    # Direct profile match (profiles use underscores)
